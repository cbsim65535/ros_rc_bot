...
JOYSTICK_CODE_VALUE_MAP = {
    ...
    "ZEROPLUS Controller": (XINPUT_CODE_MAP, XINPUT_VALUE_MAP),
}
...
class JoystickRos2(Node):
    ...
    def normalize_key_value(self, key_value_min, key_value_max, key_value):
        normalized = (
            (key_value - key_value_min) / (key_value_max - key_value_min) * (-2)
        ) + 1
                ...
                if events:
                    for event in events:
                        if event.code in JOYSTICK_CODE_VALUE_MAP[event.device.name][0]:
                            key_code = JOYSTICK_CODE_VALUE_MAP[event.device.name][0][
                                event.code
                            ]
                            if event.ev_type == "Key":
                                ...
                                value_range = JOYSTICK_CODE_VALUE_MAP[
                                    event.device.name
                                ][1][key_code]
                                ...
                                if (
                                    ...
                                    or (
                                        time.time() - self.last_publish_time
                    ...
                    break
            ## autorepeat
            if (self.autorepeat_rate > 0.0) and (
                time.time() - self.last_publish_time > 1 / self.autorepeat_rate
            ):
                self.publish_joy()
